/*	DEMO PROGRAM FOR FRDM-K22F
 *	Copyright (c) 2015 by COSMIC Software
 */
#include <iok22fn512.h>
 
#define SIM_SCGC5_PORTA_MASK	0x0200
#define SIM_SCGC5_PORTD_MASK	0x1000
#define SIM_SCGC6_FTM2_MASK	0x4000000
...
void pllinit(void)
	{
	MCG.C2 = 0x9C;				/* LOCRE0=1, RANGE=1, HGO=1, EREFS=1 */
	...
	while (!(MCG.S & MCG_S_LOCK))		/* Wait for LOCK bit to set */
		;
	MCG.C1 = 0x28;				/* CLKS=0, FRDIV=5, IREFS=0, IRCLKEN=0, IREFSTEN=0 */
 ...
 */
 void sysinit(void)
	...
	SIM.SCGC5 |= (SIM_SCGC5_PORTA_MASK |	/* Clock for PORTA */
			SIM_SCGC5_PORTD_MASK);	/* Clock for PORTD */
	SIM.SCGC6 |= SIM_SCGC6_FTM2_MASK;	/* Clock for FTM2 */

	PORTA.PCR[LED1] = MUX_GPIO;		/* LED's MUX */
	PORTA.PCR[LED2] = MUX_GPIO;
	PORTD.PCR[LED3] = MUX_GPIO;

	PTA.PDDR |= (LED1_MASK | LED2_MASK);	/* Set pin to OUTPUT */
	PTD.PDDR |= LED3_MASK;
